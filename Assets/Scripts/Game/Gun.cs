...
public class Gun : MonoBehaviour
{
    ...
    public float fireRateGun = 60.0f;
    public float fireRateShotgun = 15.0f;
    ...
    public void ShootGun(Transform director)
    {
        ...
        else
        {
            ...
            if (shotgunBullets%2 == 1)
            {
                SpawnBullet(director.position, director.rotation);
            }
            for(int i = 0; i < shotgunBullets/2; i++)
            {
                var rotationr = director.eulerAngles + new Vector3(0.0f, 0.0f, i * shotgunSpread);
                var rotationl = director.eulerAngles - new Vector3(0.0f, 0.0f, i * shotgunSpread);
                if (shotgunBullets % 2 == 1)
                {
                    rotationr = rotationr + new Vector3(0.0f, 0.0f, shotgunSpread);
                    rotationl = rotationl - new Vector3(0.0f, 0.0f, shotgunSpread);
                }
                else
                {
                    rotationr = rotationr + new Vector3(0.0f, 0.0f, shotgunSpread / 2);
                    rotationl = rotationl - new Vector3(0.0f, 0.0f, shotgunSpread / 2);
                }
                SpawnBullet(director.position, Quaternion.Euler(rotationr));
                SpawnBullet(director.position, Quaternion.Euler(rotationl));
            }
        }
    }
