...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var playerPos = GameManager.Instance.NearestPlayer(transform.position);
        if(playerPos != null)
        {
            var xDistance = transform.position.x - playerPos.transform.position.x;
            var yDistance = transform.position.y - playerPos.transform.position.y;
            var angle = 0.0f;
            if(yDistance != 0)
            {
                angle = Mathf.Atan(xDistance / yDistance);
            }
            var moveX = Mathf.Sin(angle) * 0.01f;
            var moveY = Mathf.Cos(angle) * 0.01f;
            if(transform.position.y < playerPos.transform.position.y)
            {
                moveX = -moveX;
                moveY = -moveY;
            }
            mRigidBody.MovePosition(transform.position - new Vector3(moveX, moveY, 0.0f));
            mRigidBody.MoveRotation(Quaternion.LookRotation(playerPos.transform.position - transform.position, Vector3.forward));
        }
